...
class myCoNLL:
    ...
    def load_conll(f, ignore_gapping=True):
        """ Load the file or string into the myCoNLL-U format data.
        Input: file or string reader, where the data is in myCoNLL-U format.
        ...
        for line_idx, line in enumerate(f):
            ...
            if len(line) == 0:
                ...
                assert len(array) == FIELD_NUM, \
                    f"Cannot parse myCoNLL line {line_idx + 1}: expecting {FIELD_NUM} fields, {len(array)} found.\n  {array}"
                sent += [array]
        if len(sent) > 0:
            doc.append(sent)
            doc_comments.append(sent_comments)
    ...
    def convert_conll(doc_conll):
        """ Convert the myCoNLL-U format input data to a dictionary format output data.
        Input: list of token fields loaded from the myCoNLL-U format data, where the outmost list represents a list of sentences, and the inside list represents all fields of a token.
        ...
        for sent_conll in doc_conll:
            ...
            for token_conll in sent_conll:
                token_dict = myCoNLL.convert_conll_token(token_conll)
    ...
    def convert_conll_token(token_conll):
        """ Convert the myCoNLL-U format input token to the dictionary format output token.
        Input: a list of all myCoNLL-U fields for the token.
    ...
    def conll2dict(input_file=None, input_str=None, ignore_gapping=True):
        """ Load the myCoNLL-U format data from file or string into lists of dictionaries.
        """
        ...
        doc_conll, doc_comments = myCoNLL.load_conll(infile, ignore_gapping)
        doc_dict = myCoNLL.convert_conll(doc_conll)
        return doc_dict, doc_comments

    @staticmethod
    def conll2doc(input_file=None, input_str=None, ignore_gapping=True):
        doc_dict, doc_comments = myCoNLL.conll2dict(input_file, input_str, ignore_gapping)
        return Document(doc_dict, text=None, comments=doc_comments)

    @staticmethod
    def convert_dict(doc_dict):
        """ Convert the dictionary format input data to the myCoNLL-U format output data. This is the reverse function of
        `convert_conll`.
        Input: dictionary format data, which is a list of list of dictionaries for each token in each sentence in the data.
        Output: myCoNLL-U format data, which is a list of list of list for each token in each sentence in the data.
        """
        doc_conll = []
    ...
    def convert_token_dict(token_dict):
        """ Convert the dictionary format input token to the myCoNLL-U format output token. This is the reverse function of
        `convert_conll_token`.
        Input: dictionary format token, which is a dictionaries for the token.
        Output: myCoNLL-U format token, which is a list for the token.
        """
        ...
        # modifica roberto
        #print(token_conll[FIELD_TO_IDX[NER]],file=fileError)
        if token_conll[FIELD_TO_IDX[NER]] == "O" or token_conll[FIELD_TO_IDX[NER]] == "_":
            token_conll[FIELD_TO_IDX[NER]] = "-"
        ...
        if '-' not in token_conll[FIELD_TO_IDX[ID]] and HEAD not in token_dict:
            token_conll[FIELD_TO_IDX[HEAD]] = str(int(
                token_dict[ID] if isinstance(token_dict[ID], int) else token_dict[ID][
                    0]) - 1)  # evaluation script requires head: int
        return token_conll

    @staticmethod
    def conll_as_string(doc):
        """ Dump the loaded myCoNLL-U format list data to string. """
        return_string = ""
        for sent in doc:
            for ln in sent:
    ...
    def dict2conll(doc_dict, filename):
        """ Convert the dictionary format input data to the myCoNLL-U format output data and write to a file.
        """
        doc_conll = myCoNLL.convert_dict(doc_dict)
        conll_string = myCoNLL.conll_as_string(doc_conll)
    ...
    def doc2conll(doc):
        ...
        for sentence in doc.sentences:
            ...
            for token_dict in sentence.to_dict():
                token_conll = myCoNLL.convert_token_dict(token_dict)
                #print("token_conll [in gestoreStanza]: type == ", end="")
                #print(type(token_conll))
    ...
    def doc2conll_text(doc):
        ...
        """
        doc_conll = myCoNLL.doc2conll(doc)
    ...
    def write_doc2conll(doc, filename):
        ...
        """
        with open(filename, 'w', encoding='utf-8') as outfile:
            outfile.write(myCoNLL.doc2conll_text(doc))
